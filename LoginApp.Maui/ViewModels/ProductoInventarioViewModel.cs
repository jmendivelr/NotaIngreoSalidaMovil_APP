...
namespace LoginApp.Maui.ViewModels
{
    public class ProductoInventarioViewModel
    {
        ...
        {
            get { return _cantidad_Salida; }
            set
            {
                if (_cantidad_Salida != value)
                {
                    _cantidad_Salida = value;
                    OnPropertyChanged(nameof(Cantidad_Salida));
                    //CalcularPrecioTotal();
                    //OnPropertyChanged(nameof(PrecioTotal));
                    //ActualizarTotalPreciosSeleccionados();
                }
        ...
        private int _cantidad_input;

        public int Cantidad_input
        {
            get { return _cantidad_input; }
            set
            {
                if (value == 0)
                {
                    _cantidad_input = 1;
                    OnPropertyChanged(nameof(Cantidad_input));
                }
                else if (value < Cantidad_Salida)
                {
                    _cantidad_input = value;
                    OnPropertyChanged(nameof(Cantidad_input));
                    //CalcularPrecioTotal();
                    //OnPropertyChanged(nameof(PrecioTotal));
                    //ActualizarTotalPreciosSeleccionados();
                ...
                {
                    _cantidad_input = Cantidad_Salida;
                    OnPropertyChanged(nameof(Cantidad_input));
                }
            }
        }
    ...
    public class ApiResponseProductoInventarioViewModel
    {
        public int InternalStatus { get; set; }
        public List<ProductoInventarioViewModel> Data { get; set; }
